...
export class JobDetailComponent implements OnInit {
  ...
  oneClickApply() {
    ...
    this.profileService.getResume(userId).subscribe({
      next: (resumeRes: any) => {
        ...
        const skills = this.extractSkillNames(resume.skillsList || resume.skills);
        const education = this.extractTopEducation(resume.educationList || resume.education);
        const experience = this.extractExperienceSummary(resume.experienceList || resume.experience);
        const objective = resume.objective?.objectiveStatement || '';
        const projects = this.extractProjects(resume.projectsList || resume.projects);
        const certifications = this.extractCertifications(resume.certificationsList || resume.certifications);
        ...
        const fileSection = latestFile
          ...
          : `<div class="alert alert-warning small mb-3 py-2">⚠️ No resume file uploaded. Application will use ATS data only.</div>`;
        ...
        const skillsHtml = skills
          ? `<div class="mb-2"><span class="fw-semibold">🛠️ Skills:</span><div class="mt-1">${skills.split(', ').map(s => `<span class="badge text-bg-secondary me-1 mb-1">${this.escapeHtml(s)}</span>`).join('')}</div></div>`
          : '';

        const noDataMsg = !eduList.length && !expList.length && !skills
          ? `<div class="alert alert-info small py-2 mb-2">ℹ️ Your resume builder profile has no data yet. Please fill it in the <strong>Resume Builder</strong> first.</div>`
          : '';
        ...
        Swal.fire({
          ...
          html: `
            <div class="text-start" style="font-size:0.88rem;">
              ...
              ${expHtml}
              ${skillsHtml}
              ${projects ? `<div class="mb-2"><span class="fw-semibold">🗂️ Projects:</span> <span class="text-secondary">${this.escapeHtml(projects)}</span></div>` : ''}
  ...
  private extractSkillNames(skills: unknown): string {
    ...
    return skills
      .map((item: any) => item?.skillName || item?.name || item?.skill || '')
      .filter(Boolean).join(', ');
  }
